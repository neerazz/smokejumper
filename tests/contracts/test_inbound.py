...
def test_json_form_is_base64() -> None:
    inbound = VerifiedInbound.model_validate({"source": "generic", "body": RAW_BODY})
    assert f'"body":"{ENCODED_BODY}"' in inbound.model_dump_json()


def test_a_string_body_is_read_as_base64() -> None:
    inbound = VerifiedInbound.model_validate({"source": "generic", "body": ENCODED_BODY})
    assert inbound.body == RAW_BODY


def test_a_non_base64_string_body_is_rejected() -> None:
    with pytest.raises(ValidationError):
        VerifiedInbound.model_validate({"source": "generic", "body": "not base64 at all!"})


def test_slack_needs_no_headers() -> None:
    inbound = VerifiedInbound.model_validate({"source": "slack", "body": b"{}"})
    assert inbound.headers == {}


def test_source_enum_is_closed() -> None:
    with pytest.raises(ValidationError):
        VerifiedInbound.model_validate({"source": "nagios", "body": b"{}"})
