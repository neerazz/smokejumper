...
def test_ref_round_trips_json() -> None:
    ref = TicketRef.model_validate(
        {"provider": "linear", "external_id": "SMOKE-123", "url": "https://linear.app/x/SMOKE-123"}
    )
...
def test_provider_enum_is_closed() -> None:
    with pytest.raises(ValidationError):
        TicketRef.model_validate({"provider": "trello", "external_id": "1"})
...
def test_an_update_may_carry_a_conclusion_status_or_none() -> None:
    comment_only = TicketUpdate.model_validate({"run_id": RUN_ID, "comment_md": "still firing"})
    assert comment_only.status is None
    with_status = TicketUpdate.model_validate(
        {"run_id": RUN_ID, "comment_md": "root caused", "status": "root_caused"}
    )
...
def test_a_provider_workflow_state_is_not_accepted() -> None:
    ...
    with pytest.raises(ValidationError):
        TicketUpdate.model_validate({"run_id": RUN_ID, "comment_md": "x", "status": "In Progress"})
